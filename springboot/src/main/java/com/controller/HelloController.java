...
import com.service.UserServiceSave;
...
//    // @Resource和@Autowired注解都是用来实现 依赖注入 的, 可以允许注入其他类。只是@AutoWried按by type自动注入，而@Resource默认按byName自动注入。
    ...
    public List<User> selectT(){
        return userMapper.findAllMb();
    }

    @PostMapping // 新增 更新都在这个接口里
    public boolean saveOrUpdateMbP(@RequestBody User user){ // 把json数据转换成user对象
        return userServiceSave.saveOrUpdateMBP(user) ;
    }
// @RequestBody处理前台的发送的json对象, 转换为user 格式

    @GetMapping("/list") // 另一种方式显示所有数据
    public List<User> list(){
        return userServiceSave.list(); // UserServiceSave里继承了ServiceImpl, ServiceImpl中有list方法
    }
...
//@PathVariable是路径
    ...
    public boolean deleteBatch(@RequestBody List<Integer> ids){ // 传入的ids是List 数组型,但获取到的是一整个对象，里面不仅有id 还有其他属性 (multipleSelection)
         return userServiceSave.removeByIds(ids);
    }
...
//  limit的第一个参数 ( pageNum -1 ) * pageSize, 第二个参数是 pageSize
    public Map<String, Object> findPage(@RequestParam Integer pageNum,
        ...
        List<User> data = userMapper.selectPageMb(pageNum,pageSize,username); //

        Integer total = userMapper.countTotalMb(username);
    ...
    @GetMapping("/pageMbP") // mybatis-plus实现的分页查询
...
//        queryWrapper.like("username",username);
//        queryWrapper.like("address",address); // mbp在查询时 会自动添加and 效果是 username like ? and address like ?
//        queryWrapper.like("email",email);
//      queryWrapper.or().like("email",email);  或查询 or(). 但实际上会将前两个短路 也就是其余条件失效
        return userServiceSave.page(page,queryWrapper);
    }
}